...
for x in utt2spk spk2utt feats.scp text segments wav.scp cmvn.scp vad.scp mp4.scp \
...
function filter_utts {
  ...
  for x in feats.scp text segments utt2lang mp4.scp $maybe_wav $maybe_utt2dur $maybe_utt2num_frames; do
    if [ -f $data/$x ]; then
  ...
  for x in utt2spk utt2uniq feats.scp vad.scp text segments utt2lang utt2dur utt2num_frames mp4.scp $maybe_wav $maybe_reco2dur $utt_extra_files; do
    if [ -f $data/$x ]; then
      cp $data/$x $data/.backup/$x
