...
function show_help
{
      echo "Usage: $0 [--no-feats] [--no-text] [--non-print] [--no-mp4] [--no-spk-sort] <data-dir>"
...
if [ -f $data/images.scp ]; then
  cmd=${cmd/--no-mp4/}  # remove --no-mp4 if supplied
...
if [ -f $data/segments ] && [ ! -f $data/mp4.scp ]; then
  echo "$0: in directory $data, segments file exists but no mp4.scp"
  exit 1;
fi


if [ ! -f $data/mp4.scp ] && ! $no_mp4; then
  echo "$0: no such file $data/mp4.scp (if this is by design, specify --no-mp4)"
  exit 1;
fi

if [ -f $data/mp4.scp ]; then
  check_sorted_and_uniq $data/mp4.scp

  if grep -E -q '^\S+\s+~' $data/mp4.scp; then
    # note: it's not a good idea to have any kind of tilde in mp4.scp, even if
    ...
    # it's something of the form "foo ~/foo.mp4" (i.e. a plain file name) which
    # would definitely cause problems as the fopen system call does not do
    # tilde expansion.
    echo "$0: Please do not use tilde (~) in your mp4.scp."
    exit 1;
  fi
  ...
  if [ -f $data/segments ]; then
    ...
    # We have a segments file -> interpret mp4 file as "recording-ids" not utterance-ids.
    ! cat $data/segments | \
    ...
    awk '{print $1}' $data/mp4.scp > $tmpdir/recordings.mp4
    if ! cmp -s $tmpdir/recordings{,.mp4}; then
      echo "$0: Error: in $data, recording-ids extracted from segments and mp4.scp"
      echo "$0: differ, partial diff is:"
      partial_diff $tmpdir/recordings{,.mp4}
      exit 1;
    fi
    ...
    # No segments file -> assume mp4.scp indexed by utterance.
    cat $data/mp4.scp | awk '{print $1}' > $tmpdir/utts.mp4
    if ! cmp -s $tmpdir/utts{,.mp4}; then
      echo "$0: Error: in $data, utterance lists extracted from utt2spk and mp4.scp"
      echo "$0: differ, partial diff is:"
      partial_diff $tmpdir/utts{,.mp4}
      exit 1;
    fi
...
if [ -f $data/reco2dur ]; then
  ...
  if [ -f $tmpdir/recordings ]; then
    ...
    if ! cmp -s $tmpdir/{utts,recordings.reco2dur}; then
      echo "$0: Error: in $data, recording-ids extracted from mp4.scp and reco2dur file"
      echo "$0: differ, partial diff is:"
...
if [ -f $data/mp4.scp ]; then
  echo "Validating 'mp4.scp' is to be updated."
fi
