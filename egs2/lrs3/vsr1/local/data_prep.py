...
class Utils:
    ...
    def read_text(text_file: str) -> str:
        ...
        text_val = re.sub("{[^}]*}\s*", "", text_val) # Remove unused codes
        return text_val
    ...
    def get_parser():
        ...
        parser.add_argument("--pretrain_path", type=str, required=False, help="Path to the Pretrain files")
...
class DatasetUtils:
    @staticmethod
    def train_val_files(
        train_val_path: str, train_val_ratio: float = 0.92, random_seed: int = 0, pretrain_path: Optional[str] = None,
        ...
        """
        logging.warning("function `train_val_files` is deprecated!")
        ...
        # Add pretrain paths to train paths
        if pretrain_path is not None:
            speakers_pretrain = os.listdir(pretrain_path)
            speakers_train += [
                os.path.join(pretrain_path, folder) for folder in speakers_pretrain
            ]

        return speakers_train, speakers_val

    @staticmethod
    def get_files(*test_paths: List[str]) -> List[str]:
        ...
        for test_path in test_paths:
            _speakers_test = os.listdir(test_path)
            _speakers_test = [os.path.join(test_path, folder) for folder in _speakers_test]
            speakers_test.extend(_speakers_test)
    ...
    def generate_espnet_data(
        ...
        """Generates the utt2spk, text and mp4 data required by ESPNET
        ...
        mp4 (list) : Utterence to Mp4-Path data
        """
        utt2spk = []
        text = []
        mp4 = []
        ...
        for speaker_folder in tqdm.tqdm(speaker_folders):
            ...
            for mp4_file in os.listdir(speaker_folder):
                if not mp4_file.endswith(".mp4"):
                    continue
                text_file = mp4_file.replace("mp4", "txt")
...
def main():
    ...
    train_folders = DatasetUtils.get_files(args.train_val_path, args.pretrain_path)
    test_folders = DatasetUtils.get_files(args.test_path)
    dev_folders = test_folders
