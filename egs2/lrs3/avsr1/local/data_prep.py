    ...
    def generate_espnet_data(
        ...
        for speaker_folder in tqdm.tqdm(speaker_folders):
            ...
            for wav_file in os.listdir(speaker_folder):
                ...
                assert os.path.exists(mp4_full_path), mp4_full_path
                assert os.path.exists(wav_full_path), wav_full_path
                assert os.path.exists(text_full_path), text_full_path
        ...
        return utt2spk, text, wav, mp4

    @staticmethod
    def perform_data_prep(speaker_folders: list, dataset: str) -> None:
        """Performs ESPNET related Data-Preparation.
        Generates the utt2spk, text, mp4.scp and wav.scp files

        Args:
        speaker_folders (list): The folders from where to extract data
        dataset (str): The dataset we are working with (train, test, dev)
        """
        utt2spk, text, wav, mp4 = DatasetUtils.generate_espnet_data(speaker_folders, dataset)
...
def main():
    ...
    train_folders, dev_folders = DatasetUtils.train_val_files(args.train_val_path, pretrain_path=args.pretrain_path)
