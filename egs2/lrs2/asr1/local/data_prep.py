...
class Utils:
    ...
    def get_parser():
        ...
        parser.add_argument(
            "--train_path", type=str, help="Path to the Train/ Validation or Train files"
        )
        parser.add_argument(
            "--val_path", type=str, help="Path to the Validation files"
        )
...
class DatasetUtils:
    @staticmethod
    def train_val_files(
        train_path: str, train_val_ratio: float = 0.92, random_seed: int = 0, 
        val_path: Optional[str] = None, pretrain_path: Optional[str] = None,
        ...
        """
        if val_path is None:
            speaker_folders = os.listdir(train_path)
            ...
            speakers_train = [
                os.path.join(train_path, folder) for folder in speakers_train
            ]
            speakers_val = [os.path.join(train_path, folder) for folder in speakers_val]
        else:
            speakers_train = [os.path.join(train_path, folder) for folder in os.listdir(train_path)]
            speakers_val = [os.path.join(val_path, folder) for folder in os.listdir(val_path)]
...
def main():
    ...
    train_folders, dev_folders = DatasetUtils.train_val_files(args.train_path, val_path=args.val_path, pretrain_path=args.pretrain_path)
