...
from espnet2.avsr.encoder.late_fusion import UnimodalLateFusionEncoder
...
from espnet2.avsr.encoder.abs_encoder import AbsFusionEncoder
from espnet2.avsr.layers.fusion import AbsFusion, ConcatMLPFusion, ConcatTemporalFusion, RelScoreFusion
...
fusion_choices = ClassChoices(
    ...
    classes=dict(
        mlp=ConcatMLPFusion,
        temporal=ConcatTemporalFusion,
        relscore=RelScoreFusion,
    ),
    type_check=AbsFusion,
    default='mlp',
)
wrapper_choices = ClassChoices(
    name="wrapper",
    classes=dict(
        late=UnimodalLateFusionEncoder,
    ),
...
class IIPnetASRModel(AbsESPnetModel):
    ...
    def __init__(
        ...
        fusion_conf: Optional[dict]={},
        wrapper_conf: Optional[dict]={},
        aux_ctc: dict = None,
        ...
        fusion_class = fusion_choices.get_class(fusion)
        fusion_method = fusion_class(**fusion_conf)

        from copy import deepcopy
        wrapper_class = wrapper_choices.get_class(wrapper)
        wrapper = wrapper_class(
            vocab_size=vocab_size,
            audioFrontend=frontend,
            audioSpecaug=specaug,
            audioNormalize=normalize,
            audioPreencoder=preencoder,
            audioPreencoder2=deepcopy(preencoder),
            audioEncoder=encoder,
            audioEncoder2=deepcopy(encoder),
            audioPostencoder=postencoder,
            audioPostencoder2=deepcopy(postencoder),
            fusionModule=fusion_method,
            **wrapper_conf,
        )
    ...
    def collect_feats(self, **batch: torch.Tensor) -> Dict[str, torch.Tensor]:
        return self.encoder.collect_feats(**batch)
