...
def cut_or_pad(data, size, dim=0):
    ...
    """
    if data.shape[dim] < size:
        padding = size - data.shape[dim]
        if data.ndim > 1:
            pad_tuple = [(0,padding)] + [(0,0) for _ in range(data.ndim-1)]
            data = np.pad(data, pad_tuple, mode="constant")
        else:
            data = np.pad(data, (0, padding), mode="constant")
    elif data.shape[dim] > size:
        data = data[:size]
    assert data.shape[dim] == size
    return data

def interpolate_1d(data, size, dim=0):
    """
    Interpolate the data to match along a dimension.
    """
    from scipy.ndimage import zoom
    factor = size / data.shape[dim]
    factors = [1 for _ in range(data.ndim)]
    factors[dim] = factor
...
class CommonPreprocessor(AbsPreprocessor):
    def __init__(
        ...
        noise_db_infer: float = None,
        noise_saved_dir: str = None,
        short_noise_thres: float = 0.5,
        aux_task_names: Collection[str] = None,
        speech_speed_perturb: str = "1.0",
        speech_volume_normalize: float = None,
        adaptive_time_mask: bool = False,
        adaptive_time_mask_period: float = 1.,
        adaptive_time_mask_rate: float = 0.4,
        ...
        self.sample_rate = fs
        self.sp_factors = [float(sp) for sp in speech_speed_perturb.split("_")]
        logging.info(f"Uniformly random speed perturbation with the factors;{self.sp_factors}")
        self.adaptive_time_mask = adaptive_time_mask
        self.adaptive_time_mask_period = adaptive_time_mask_period
        self.adaptive_time_mask_rate = adaptive_time_mask_rate
        self.speech_volume_normalize = speech_volume_normalize
        self.rir_apply_prob = rir_apply_prob
        self.noise_apply_prob = noise_apply_prob
        self.short_noise_thres = short_noise_thres
        self.noise_db_infer = noise_db_infer
        self.noise_saved_dir = noise_saved_dir
        ...
        if train and noise_scp is not None:
            ...
            if len(sps) == 1:
                ...
                self.noise_db_list = [float(sp) for sp in sps]
            else:
                raise ValueError(
    ...
    def _add_noise(self, speech, power):
        ...
        if noise_path is not None:
            if hasattr(self, 'noise_db_list'):
                noise_db = np.random.choice(self.noise_db_list)
            else:
    ...
    def _add_noise_fixed(self, uid, speech, power):
        import os
        assert self.noise_db_infer is not None
        noise_db = self.noise_db_infer
        noise_path = os.path.join(self.noise_saved_dir, uid+'.wav')
    ...
    def _add_mask(self, speech):
        # speech: (T, ...)
        window = self.adaptive_time_mask_rate * self.sample_rate
        stride = self.adaptive_time_mask_period * self.sample_rate
        
        length = speech.shape[0]
        n_mask = int((length + stride - 0.1) // stride)
        ts = np.random.randint(0, window, size=(n_mask, 2),)
        for t, t_end in ts:
            if length - t <= 0:
                continue
            ...
            if t_start == t_start + t:
                continue
            t_end += t_start
            speech[t_start:t_end] = 0
        
        return speech

    def _speech_process(
        self, data: Dict[str, Union[str, np.ndarray]], uid: str=None
        ...
        if self.speech_name in data:
            if self.train:
                ...
                # 0. Adaptive time masking
                if self.adaptive_time_mask:
                    speech = self._add_mask(speech)
                
                # 0. Speed perturbation
                sp_factor = np.random.choice(self.sp_factors)
                if sp_factor != 1.0:
                    speech = time_stretch(speech, rate=sp_factor)
                ...
                # NOTE(JK) expand dims during validation or evaluation step.
                speech = data[self.speech_name]
                if speech.ndim == 1:
                    speech = speech[:,None]
                
                # 2. Add Noise
                if self.noise_db_infer is not None:
                    # Calc power on non silence region
                    power = (speech[detect_non_silence(speech)] ** 2).mean()
                    speech = self._add_noise_fixed(uid, speech, power)
                
                data[self.speech_name] = speech
                # soundfile.write(f"noise_syn/{uid}.wav", data[self.speech_name], samplerate=16000)
            ...
            assert data[self.speech_name].ndim==2, (data[self.speech_name].shape, self.train, self.rirs, self.noises)
    ...
    def __call__(
        ...
        data = self._speech_process(data, uid)
        data = self._text_process(data)
        return data


class VisualPreprocessor(AbsPreprocessor):
    def __init__(
        self,
        train: bool,
        token_type: str = None,
    ...
    def _add_vmask(self, video):
        # video: (T, ...)
        window = self.adaptive_time_mask_rate * self.fps
        stride = self.adaptive_time_mask_period * self.fps
    ...
    def _visual_process(
        ...
        """Preprocess for visual data. 
        e.g. normalization, noise synthesis, ..."""
        assert check_return_type(data)
        if self.visual_name in data:
            if self.train:
                video = data[self.visual_name]
                # 0. Adaptive time masking
                if self.adaptive_time_mask:
                    video = self._add_vmask(video)
                data[self.visual_name] = video
        return data
    ...
    def __call__(
        ...
        data = self._visual_process(data)
        data = self._text_process(data)
        return data
   

class AudioVisualPreprocessor(CommonPreprocessor, VisualPreprocessor):
    def __init__(
        self, 
        train: bool, token_type: str = None, token_list: Union[Path, str, Iterable[str]] = None, 
        bpemodel: Union[Path, str, Iterable[str]] = None, text_cleaner: Collection[str] = None, 
        g2p_type: str = None, unk_symbol: str = "<unk>", space_symbol: str = "<space>", 
        non_linguistic_symbols: Union[Path, str, Iterable[str]] = None, 
        delimiter: str = None, rir_scp: str = None, rir_apply_prob: float = 1, 
        noise_scp: str = None, noise_apply_prob: float = 1, 
        noise_db_range: str = "3_10", noise_db_infer: float = None, noise_saved_dir: str = None,
        short_noise_thres: float = 0.5, aux_task_names: Collection[str] = None, 
        speech_volume_normalize: float = None, adaptive_time_mask: str = False, 
        adaptive_time_mask_period: float = 1., adaptive_time_mask_rate: float = 0.4, 
        speech_name: str = "speech", visual_name: str = "visual", text_name: str = "text", 
        fs: int = 16000, fps: int = 0, nonsplit_symbol: Iterable[str] = None,
        align_protocol: str = "cut_or_pad", align_side: str = "visual",
        dropout_modality: float = 0.0):
        CommonPreprocessor.__init__(
            self,
            train=train, token_type=token_type, token_list=token_list, 
            bpemodel=bpemodel, text_cleaner=text_cleaner, 
            g2p_type=g2p_type, unk_symbol=unk_symbol, space_symbol=space_symbol, 
            non_linguistic_symbols=non_linguistic_symbols, 
            delimiter=delimiter, rir_scp=rir_scp, rir_apply_prob=rir_apply_prob, 
            noise_scp=noise_scp, noise_apply_prob=noise_apply_prob, 
            noise_db_range=noise_db_range, noise_db_infer=noise_db_infer, noise_saved_dir=noise_saved_dir,
            short_noise_thres=short_noise_thres, aux_task_names=aux_task_names, 
            speech_volume_normalize=speech_volume_normalize, 
            adaptive_time_mask=adaptive_time_mask, adaptive_time_mask_period=adaptive_time_mask_period,
            adaptive_time_mask_rate=adaptive_time_mask_rate, speech_name=speech_name, text_name=text_name, 
            fs=fs, nonsplit_symbol=nonsplit_symbol)
        VisualPreprocessor.__init__(
            self,
            train=train, token_type=token_type, token_list=token_list, 
            bpemodel=bpemodel, text_cleaner=text_cleaner, 
            g2p_type=g2p_type, unk_symbol=unk_symbol, space_symbol=space_symbol, 
            non_linguistic_symbols=non_linguistic_symbols, 
            delimiter=delimiter, aux_task_names=aux_task_names, 
            adaptive_time_mask=adaptive_time_mask, adaptive_time_mask_period=adaptive_time_mask_period,
            adaptive_time_mask_rate=adaptive_time_mask_rate, 
            visual_name=visual_name, text_name=text_name, 
            fps=fps, nonsplit_symbol=nonsplit_symbol)
        self.align_protocol = align_protocol
        self.align_side = align_side
        self.dropout_modality = dropout_modality

    def _dropout_modality(
        self, data: Dict[str, Union[str, np.ndarray]]
    ) -> Dict[str, np.ndarray]:
        drop_target = np.random.choice([self.speech_name, self.visual_name])
        data[drop_target] = np.zeros_like(data[drop_target])
        return data
 
    def _align(
        self, uid: str, data: Dict[str, Union[str, np.ndarray]]
    ) -> Dict[str, np.ndarray]:
        assert self.align_side in ["visual", "speech"]
        speech = data[self.speech_name]
        visual = data[self.visual_name]
        exp_factor = int(self.sample_rate / self.fps)
        if self.align_side == "visual":
            src = speech
            src_name = self.speech_name
            src_length = speech.shape[0]
            target_length = len(visual) * exp_factor
            min_discrepancy = 1 * self.sample_rate
        elif self.align_side == "speech":
            src = visual
            src_name = self.visual_name
            src_length = visual.shape[0]
            target_length = len(speech) // exp_factor
            min_discrepancy = 1 * self.fps
        
        if src_length == target_length:
            return data
        elif abs(src_length - target_length) > min_discrepancy:
            logging.warning(f"Lengths of data in [uid]:{uid} differs {abs(src_length - target_length) / min_discrepancy:.2f} in seconds, more than 1 second.")
            logging.warning(f"{speech.shape, visual.shape, exp_factor, target_length, min_discrepancy}")
        else:
            # logging.info(f"{src_length - target_length}")
            pass
        
        if self.align_protocol is None:
            return data
        elif self.align_protocol == 'cut_or_pad':
            data[src_name] = cut_or_pad(src, target_length)
            return data
        elif self.align_protocol == 'interpolate':
            data[src_name] = interpolate_1d(src, target_length)
            return data
        else:
            raise NotImplementedError
    ...
    def __call__(
        ...
        data = self._visual_process(data)
        data = self._speech_process(data, uid)
        if self.dropout_modality > np.random.random():
            data = self._dropout_modality(data)
        data = self._align(uid, data)
        data = self._text_process(data)
        return data
