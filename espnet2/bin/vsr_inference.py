...
class Speech2Text:
    ...
    Examples:
        >>> import soundfile
        >>> visual2text = Speech2Text("vsr_config.yml", "vsr.pth")
        >>> visual, rate = soundfile.read("visual.wav")
        >>> visual2text(visual)
    ...
    def __init__(
        self,
        vsr_train_config: Union[Path, str] = None,
        vsr_model_file: Union[Path, str] = None,
        ...
        if quantize_vsr_model or quantize_lm:
        ...
        # 1. Build VSR model
        scorers = {}
        vsr_model, vsr_train_args = task.build_model_from_file(
            vsr_train_config, vsr_model_file, device
        )

        vsr_model.to(dtype=getattr(torch, dtype)).eval()

        if quantize_vsr_model:
            logging.info("Use quantized vsr model for decoding.")

            vsr_model = torch.quantization.quantize_dynamic(
                vsr_model, qconfig_spec=quantize_modules, dtype=quantize_dtype
            )

        decoder = vsr_model.decoder

        ctc = CTCPrefixScorer(ctc=vsr_model.ctc, eos=vsr_model.eos)
        token_list = vsr_model.token_list
        scorers.update(
            decoder=decoder,
            ctc=ctc,
        ...
        if vsr_model.use_transducer_decoder:
            ...
            multi_blank_durations = getattr(
                vsr_model, "transducer_multi_blank_durations", []
            )[::-1] + [1]
            multi_blank_indices = [
                vsr_model.blank_id - i + 1
            ...
            beam_search_transducer = BeamSearchTransducer(
                decoder=vsr_model.decoder,
                joint_network=vsr_model.joint_network,
                beam_size=beam_size,
            ...
            del vsr_model.decoder.lm_head
            del vsr_model.decoder.decoder
                ...
                if batch_size == 1:
                    ...
                    if len(non_batch) == 0:
                        if streaming:
                            ...
                            beam_search.set_streaming_config(vsr_train_config)
                            logging.info(
        ...
        if token_type is None:
            token_type = vsr_train_args.token_type
        if bpemodel is None:
            bpemodel = vsr_train_args.bpemodel
        ...
        self.vsr_train_args = vsr_train_args
    ...
    @torch.no_grad()
    def __call__(
        self, visual: Union[torch.Tensor, np.ndarray]
    ) -> Union[
        ListOfHypothesis,
        Tuple[
            ListOfHypothesis,
        ...
        # lengths: (1,)
        lengths = visual.new_full([1], dtype=torch.long, fill_value=visual.size(1))
        batch = {"visual": visual, "visual_lengths": lengths}
        ...
        # b. Forward Encoder
        enc, enc_olens = self.vsr_model.encode(**batch)
    ...
    def _decode_interctc(
        ...
        exclude_ids = [self.vsr_model.blank_id, self.vsr_model.sos, self.vsr_model.eos]
        res = {}
        token_list = self.beam_search.token_list

        for layer_idx, encoder_out in intermediate_outs:
            y = self.vsr_model.ctc.argmax(encoder_out)[0]  # batch_size = 1
    ...
    def _decode_single_sample(self, enc: torch.Tensor):
        ...
        for hyp in nbest_hyps:
            ...
            last_pos = None if self.vsr_model.use_transducer_decoder else -1
...
def inference(
    ...
    vsr_train_config: Optional[str],
    vsr_model_file: Optional[str],
    ...
    # 2. Build visual2text
    visual2text_kwargs = dict(
        vsr_train_config=vsr_train_config,
        vsr_model_file=vsr_model_file,
        ...
        nbest=nbest,
        streaming=streaming,
        quantize_vsr_model=quantize_vsr_model,
    ...
    visual2text = Speech2Text.from_pretrained(
        model_tag=model_tag,
        **visual2text_kwargs,
    )

    # 3. Build data-iterator
    loader = VSRTask.build_streaming_iterator(
        data_path_and_name_and_type,
        dtype=dtype,
        batch_size=batch_size,
        key_file=key_file,
        num_workers=num_workers,
        preprocess_fn=VSRTask.build_preprocess_fn(visual2text.vsr_train_args, False),
        collate_fn=VSRTask.build_collate_fn(visual2text.vsr_train_args, False),
...
def get_parser():
    ...
    group.add_argument(
        "--vsr_train_config",
        type=str,
        help="VSR training configuration",
    )
    group.add_argument(
        "--vsr_model_file",
        type=str,
        help="VSR model parameter file",
    )
    group.add_argument(
        "--lm_train_config",
        type=str,
    ...
    group.add_argument(
        ...
        type=str2bool,
        default=False,
        help="Apply dynamic quantization to VSR model.",
    )
    group.add_argument(
        "--quantize_lm",
        type=str2bool,
    ...
    group.add_argument(
        ...
        help="The token type for VSR model. "
