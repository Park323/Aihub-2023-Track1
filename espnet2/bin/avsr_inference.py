...
class SpeechVisual2Text:
    """SpeechVisual2Text class

    Examples:
        >>> import soundfile
        >>> import cv2
        >>> av2text = SpeechVisual2Text("avsr_config.yml", "avsr.pth")
        >>> audio, rate = soundfile.read("speech.wav")
        >>> visual, = ??
        >>> av2text(audio, visual)
    ...
    def __init__(
        self,
        avsr_train_config: Union[Path, str] = None,
        avsr_model_file: Union[Path, str] = None,
        ...
        quantize_avsr_model: bool = False,
        ...
        multi_avsr: bool = False,
    ):
        assert check_argument_types()

        task = AVSRTask if not enh_s2t_task else EnhS2TTask

        if quantize_avsr_model or quantize_lm:
        ...
        # 1. Build AVSR model
        scorers = {}
        avsr_model, avsr_train_args = task.build_model_from_file(
            avsr_train_config, avsr_model_file, device
        )

        if enh_s2t_task:
            raise NotImplementedError
        avsr_model.to(dtype=getattr(torch, dtype)).eval()

        if quantize_avsr_model:
            logging.info("Use quantized avsr model for decoding.")

            avsr_model = torch.quantization.quantize_dynamic(
                avsr_model, qconfig_spec=quantize_modules, dtype=quantize_dtype
            )

        decoder = avsr_model.decoder

        ctc = CTCPrefixScorer(ctc=avsr_model.ctc, eos=avsr_model.eos)
        token_list = avsr_model.token_list
        scorers.update(
            decoder=decoder,
            ctc=ctc,
        ...
        if avsr_model.use_transducer_decoder:
            ...
            multi_blank_durations = getattr(
                avsr_model, "transducer_multi_blank_durations", []
            )[::-1] + [1]
            multi_blank_indices = [
                avsr_model.blank_id - i + 1
            ...
            beam_search_transducer = BeamSearchTransducer(
                decoder=avsr_model.decoder,
                joint_network=avsr_model.joint_network,
                beam_size=beam_size,
            ...
            del avsr_model.decoder.lm_head
            del avsr_model.decoder.decoder
                ...
                if batch_size == 1:
                    ...
                    if len(non_batch) == 0:
                        if streaming:
                            ...
                            beam_search.set_streaming_config(avsr_train_config)
                            logging.info(
        ...
        if token_type is None:
            token_type = avsr_train_args.token_type
        if bpemodel is None:
            bpemodel = avsr_train_args.bpemodel
        ...
        self.avsr_model = avsr_model
        self.avsr_train_args = avsr_train_args
        ...
        self.multi_avsr = multi_avsr

    @torch.no_grad()
    def __call__(
        self, 
        speech: Union[torch.Tensor, np.ndarray], 
        visual: Union[torch.Tensor, np.ndarray],
    ) -> Union[
        ListOfHypothesis,
        Tuple[
            ListOfHypothesis,
        ...
        Args:
            data: Input speech and visual data
        Returns:
        ...
        # Input as visual signal
        if isinstance(visual, np.ndarray):
            visual = torch.tensor(visual)

        # audio data: (Nsamples,) -> (1, Nsamples)
        speech = speech.unsqueeze(0).to(getattr(torch, self.dtype))
        # visual data: (Nsamples, *) -> (1, Nsamples, *)
        visual = visual.unsqueeze(0).to(getattr(torch, self.dtype))
        # lengths: (1,)
        speech_lengths = speech.new_full([1], dtype=torch.long, fill_value=speech.size(1))
        visual_lengths = visual.new_full([1], dtype=torch.long, fill_value=visual.size(1))
        batch = {
            "speech": speech, "speech_lengths": speech_lengths,
            "visual": visual, "visual_lengths": visual_lengths,}
        logging.info("speech length: " + str(speech.size(1)))
        logging.info("visual length: " + str(visual.size(1)))

        # a. To device
        batch = to_device(batch, device=self.device)

        # b. Forward Encoder
        enc, enc_olens = self.avsr_model.encode(**batch)
        if self.multi_avsr:
            raise NotImplementedError
        if self.enh_s2t_task or self.multi_avsr:
            raise NotImplementedError
        else:
            # Normal AVSR
    ...
    def _decode_interctc(
        ...
        exclude_ids = [self.avsr_model.blank_id, self.avsr_model.sos, self.avsr_model.eos]
        res = {}
        token_list = self.beam_search.token_list

        for layer_idx, encoder_out in intermediate_outs:
            y = self.avsr_model.ctc.argmax(encoder_out)[0]  # batch_size = 1
    ...
    def _decode_single_sample(self, enc: torch.Tensor):
        ...
        for hyp in nbest_hyps:
            ...
            last_pos = None if self.avsr_model.use_transducer_decoder else -1
    ...
    def from_pretrained(
        ...
        **kwargs: Optional[Any],
        ...
        """Build SpeechVisual2Text instance from the pretrained model.
        ...
        Returns:
            SpeechVisual2Text: SpeechVisual2Text instance.
        ...
        return SpeechVisual2Text(**kwargs)
...
def inference(
    ...
    avsr_train_config: Optional[str],
    avsr_model_file: Optional[str],
    ...
    # 2. Build av2text
    av2text_kwargs = dict(
        avsr_train_config=avsr_train_config,
        avsr_model_file=avsr_model_file,
        ...
        multi_avsr=multi_avsr,
        quantize_avsr_model=quantize_avsr_model,
    ...
    av2text = SpeechVisual2Text.from_pretrained(
        model_tag=model_tag,
        **av2text_kwargs,
    )
    ...
    loader = AVSRTask.build_streaming_iterator(
        ...
        preprocess_fn=AVSRTask.build_preprocess_fn(av2text.avsr_train_args, False),
        collate_fn=AVSRTask.build_collate_fn(av2text.avsr_train_args, False),
    ...
    with DatadirWriter(output_dir) as writer:
        for keys, batch in loader:
            ...
            try:
                results = av2text(**batch)
            except TooShortUttError as e:
                logging.warning(f"Utterance {keys} {e}")
                hyp = Hypothesis(score=0.0, scores={}, states={}, yseq=[])
                results = [[" ", ["<space>"], [2], hyp]] * nbest
                if enh_s2t_task:
                    num_spk = getattr(av2text.avsr_model.enh_model, "num_spk", 1)
                    results = [results for _ in range(num_spk)]

            # Only supporting batch_size==1
            key = keys[0]
            if enh_s2t_task or multi_avsr:
                raise NotImplementedError
            else:
                # Normal AVSR
...
def get_parser():
    ...
    group.add_argument(
        "--avsr_train_config",
        type=str,
        help="AVSR training configuration",
    )
    group.add_argument(
        "--avsr_model_file",
        type=str,
        help="AVSR model parameter file",
    )
    group.add_argument(
        "--lm_train_config",
        type=str,
    ...
    group.add_argument(
        ...
        type=str2bool,
        default=False,
        help="enhancement and avsr joint model",
    )
    group.add_argument(
        "--multi_avsr",
        type=str2bool,
        default=False,
        help="multi-speaker avsr model",
    )
    ...
    group.add_argument(
        ...
        type=str2bool,
        default=False,
        help="Apply dynamic quantization to AVSR model.",
    )
    group.add_argument(
        "--quantize_lm",
        type=str2bool,
    ...
    group.add_argument(
        ...
        help="The token type for AVSR model. "
